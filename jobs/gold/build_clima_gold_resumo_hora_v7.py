...
GOLD_PATH = "s3a://urbanflow-datalake-dev-us-east-1-139961319000/urbanflow/gold/clima_resumo_hora_v2/"

spark = (
    SparkSession.builder
    .appName("urbanflow-build-clima-gold-resumo-hora-v7")
    .getOrCreate()
)

print("[GOLD-CLIMA-V2] Lendo silver...")
...
df_tratado = (
    ...
    .withColumn("condicao_extraida", F.get_json_object(F.col("condicao"), "$.condicao"))
    .withColumn("temperatura_c", F.get_json_object(F.col("condicao"), "$.temperatura_c").cast("double"))
    .withColumn("chuva_mm_h", F.get_json_object(F.col("condicao"), "$.chuva_mm_h").cast("double"))
...
gold_df = (
    df_tratado
    .groupBy("cidade_tratada", "condicao_extraida", "dt_ref", "hora_ref")
    .agg(
        ...
        F.sum("chuva_mm_h").alias("precipitacao_total")
    )
    .withColumn("gold_process_ts", F.current_timestamp())
    .select(
        F.col("cidade_tratada").alias("cidade"),
        F.col("condicao_extraida").alias("condicao_climatica"),
        "qtd_eventos_clima",
        "temperatura_media",
        "temperatura_max",
        "temperatura_min",
...
print("[GOLD-CLIMA-V2] Prévia final:")
gold_df.show(20, truncate=False)

print("[GOLD-CLIMA-V2] Gravando no S3...")
(
    gold_df.write
    .mode("overwrite")
    .parquet(GOLD_PATH)
...
print("[GOLD-CLIMA-V2] Reconstrução concluída com sucesso.")
